...
begin
  ...
  try
    ...
    begin
      InitialTime := StringToTime(Copy(tmpSubFile[i], length(tmpSubFile[i])-24, 11), FALSE, TRUE);    // True for frames
      FinalTime   := StringToTime(Copy(tmpSubFile[i], length(tmpSubFile[i])-10, 11), FALSE, TRUE);    // True for frames
