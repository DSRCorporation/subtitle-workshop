...
// File Description: SubStation Alpha subtitle format reading functionality
...
function FileToSubtitles_SUBSTATIONALPHA(var Subtitles: TSubtitles; tmpSubFile: TSubtitleFile; ExtraTime: Integer): Boolean;
  ...
  begin
    ...
    SSAFile.Text := tmpSubFile.Text;
	  ...
	  Color := ColorToHtml(StrToInt(Color));
    Result := Color;
  end;
  //added by adenry: end
  ...
  function ConvertToSWTags(Text: String; PrimaryColour: String): String;
    ...
    //added by adenry: begin 2013.04.11
		function SSATagsToSWTags(Text, DefaultSSATextColor: String): String;
		var
		...
		begin
		  ...
		  begin
		    ...
		    begin
		      ...
		      begin
		        //check for bold, italic and underline - although SSA does NOT support UNDERLINE tags, we check for them just in case
		        ...
		        begin
		          ...
		          if (Copy(origTag, subtagPos+1, 3) = 'c&H') then
		            c := 4 else
		          if (Copy(origTag, subtagPos+1, 4) = '1c&H') then //although SSA does NOT support 1c tags, we check for them just in case
		            c := 5;
		          if c > 0 then
		          begin
...
// removed by adenry: end
      ...
      begin
        Text := SSATagsToSWTags(Text, PrimaryColour);
        {$IFNDEF VIPLAY}
  //SINGLE TAGS MODE
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        Text := tmpSubFile[i]; //added by adenry
        // Always 9 commas before text
        for a := 1 to 9 do
          Delete(Text, 1, Pos(',', Text)); //modified by adenry //tmpSubFile[i] := Copy(tmpSubFile[i], Pos(',',tmpSubFile[i]) + 1, Length(tmpSubFile[i]));

        Text := ReplaceString(Trim(Text), '\N', #13#10); //modified by adenry //Text := ReplaceString(Trim(tmpSubFile[i]), '\N', #13#10);
